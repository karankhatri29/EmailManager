...
def _search(db, user_id, query, plan=None, query_vector=None, **kwargs):
    ...
    with (
        patch.object(search_service.ai_summarizer, "generate_json", return_value=plan)
        if plan is not None
...
def test_make_plan_tells_the_model_todays_date_and_the_valid_categories():
    with patch.object(
        search_service.ai_summarizer, "generate_json", return_value=_ai_plan(keywords=["x"])
    ) as gen:
...
def test_make_plan_ignores_nonsense_from_the_model():
    raw = _ai_plan(
        keywords=["couch"], date_from="last summer", date_to="2026-13-45", category="Very Important"
    )
    with patch.object(search_service.ai_summarizer, "generate_json", return_value=raw):
        plan = make_plan("couch", TODAY)
    assert (
        plan.date_from is None
        and plan.date_to is None
        and plan.category is None
        and plan.keywords == ["couch"]
    )
...
def test_plan_explanation_is_human_readable():
    ...
    assert (
        "couch" in text
        and "from ikea" in text
        and "Important" in text
        and "2026-06-01" in text
        and "2026-08-31" in text
    )
...
def test_keyword_only_search_ranks_better_matches_first(db, account):
    repo.upsert_many(
        ...
        [
            ...
            stored_email(
                account, "partly", subject="Couch order", sender="Invoices Team <inv@x.com>", body="thanks"
            ),
...
def test_date_range_from_the_question_limits_the_candidates(db, account):
    ...
    repo.upsert_many(
        ...
        [
            ...
            stored_email(
                account, "spring", subject="Couch invoice", date=datetime(2026, 4, 1, tzinfo=timezone.utc)
            ),
            stored_email(
                account,
                "last-day",
                subject="Couch invoice",
                date=datetime(2026, 8, 31, 23, 0, tzinfo=timezone.utc),
            ),
            stored_email(
                account, "after", subject="Couch invoice", date=datetime(2026, 9, 1, tzinfo=timezone.utc)
            ),
        ],
    )
...
def test_sender_and_category_from_the_question_filter_too(db, account):
    repo.upsert_many(
        ...
        [
            ...
            stored_email(
                account, "promo", sender="IKEA <hi@ikea.com>", subject="Order", category="Promotional"
            ),
        ],
    )
...
def test_meaning_finds_mail_that_shares_no_words_with_the_question(db, account):
    ...
    result = _search(
        ...
        query_vector=embeddings.unpack(_vec(1.0, 0.0)),
    )
...
def test_words_and_meaning_together_beat_either_alone(db, account):
    ...
    assert _ids(result) == [
        "both"
    ]  # matches on words AND meaning; the others match only one and fall below the cutoff
...
def test_emails_without_vectors_are_still_found_by_words(db, account):
    repo.upsert_many(
        ...
        [
            ...
            stored_email(account, "other", subject="Lunch"),
        ],
    )
    result = _search(
        db,
        ...
        "couch",
        plan=_ai_plan(keywords=["couch"]),
        query_vector=embeddings.unpack(_vec(1.0)),
    )
...
def test_searches_only_the_users_own_mail_including_handled_mail(db, user, bob, account, bob_account):
    repo.upsert_many(
        db,
        [
            stored_email(account, "mine", subject="Couch invoice", is_done=True),
            stored_email(bob_account, "bobs", subject="Couch invoice"),
        ],
    )
...
def test_mailbox_filter_and_limit(db, user, account):
    ...
    repo.upsert_many(
        db,
        [stored_email(account, str(i), subject="Couch") for i in range(4)]
        + [stored_email(other, "x", subject="Couch")],
    )
    plan = _ai_plan(keywords=["couch"])
...
def test_works_end_to_end_without_any_ai(db, account):
    repo.upsert_many(
        db,
        [
            stored_email(account, "a", subject="Blue couch invoice"),
            stored_email(account, "b", subject="Lunch"),
        ],
    )
...
def test_very_long_questions_are_truncated(db, account):
    repo.upsert_many(
        db, [stored_email(account, "a", subject="x")]
    )  # something to rank, so the query is embedded
    with (
        patch.object(
            search_service.ai_summarizer, "generate_json", return_value=_ai_plan(keywords=["x"])
        ) as gen,
        patch.object(search_service.embeddings, "embed_query", return_value=None) as embed,
    ):
