...
def test_lists_bulk_senders_busiest_first(auth_client, db, account):
    ...
    assert [g["sender_address"] for g in auth_client.get("/api/newsletters", params={"days": 90}).json()][-1] == "old@x.com"
    assert len(auth_client.get("/api/newsletters", params={"min_count": 1}).json()) == 3  # adds the single-mail sender
...
def test_a_non_promotional_sender_with_an_unsubscribe_link_counts_as_bulk(auth_client, db, account):
    ...
    _seed(db, account, ("a", {"sender": "Digest <d@digest.com>", "date": _now(days=-1), **link}), ("b", {"sender": "Digest <d@digest.com>", "date": _now(days=-2), **link}))
...
def test_one_click_unsubscribe_and_mute(auth_client, db, account):
    ...
    assert r.json() == {"method": "one_click", "url": None, "ok": True, "detail": "Unsubscribed.", "muted": True}
    one_click.assert_called_once_with("https://shop.com/u")
    rules = auth_client.get("/api/rules").json()
    assert [(x["kind"], x["pattern"], x["category"]) for x in rules] == [("sender", "deals@shop.com", "Promotional")]
...
def test_without_one_click_the_link_is_handed_back_to_open(auth_client, db, account):
    ...
    with patch(ONE_CLICK) as one_click:
        r = auth_client.post("/api/newsletters/unsubscribe", json={"sender_address": "deals@shop.com", "mute": False})
    body = r.json()
    assert body["method"] == "link" and body["url"] == "https://shop.com/u" and body["ok"] is True and body["muted"] is False
...
def test_mailto_links_are_handed_back_too(auth_client, db, account):
    _seed(db, account, ("a", _promo(unsubscribe_url="mailto:unsub@shop.com?subject=stop")), ("b", _promo(days=2)))
...
def test_failed_one_click_falls_back_to_the_link(auth_client, db, account):
    _seed(db, account, ("a", _promo(unsubscribe_url="https://shop.com/u", unsubscribe_one_click=True)), ("b", _promo(days=2)))
    for outcome in (False, requests.ConnectionError("down")):
        kwargs = {"side_effect": outcome} if isinstance(outcome, Exception) else {"return_value": outcome}
        with patch(ONE_CLICK, **kwargs):
            body = auth_client.post("/api/newsletters/unsubscribe", json={"sender_address": "deals@shop.com", "mute": False}).json()
...
def test_unsafe_links_are_not_contacted_and_the_user_is_told(auth_client, db, account):
    _seed(db, account, ("a", _promo(unsubscribe_url="https://evil.example/u", unsubscribe_one_click=True)), ("b", _promo(days=2)))
    with patch(ONE_CLICK, side_effect=UnsafeUrl("Refusing to contact a private or internal address")):
        body = auth_client.post("/api/newsletters/unsubscribe", json={"sender_address": "deals@shop.com", "mute": False}).json()
    assert body["method"] == "none" and body["ok"] is False and "not safe" in body["detail"] and body["url"] is None


def test_muting_reclassifies_the_senders_mail(auth_client, db, account):
    _seed(db, account, ("a", {"sender": SHOP, "category": "General", "unsubscribe_url": "https://shop.com/u", "date": _now(days=-1)}), ("b", {"sender": SHOP, "category": "General", "unsubscribe_url": "https://shop.com/u", "date": _now(days=-2)}))
...
def test_unknown_sender_is_404_and_users_are_isolated(auth_client, bob_client, db, account, bob_account):
    _seed(db, account, ("a", _promo(unsubscribe_url="https://shop.com/u")), ("b", _promo(days=2)))
    assert auth_client.post("/api/newsletters/unsubscribe", json={"sender_address": "nobody@x.com"}).status_code == 404
    assert bob_client.post("/api/newsletters/unsubscribe", json={"sender_address": "deals@shop.com"}).status_code == 404
