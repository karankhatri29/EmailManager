...
def test_search_returns_the_plan_and_ranked_results(auth_client, db, account):
    ...
    with (
        patch(PLAN, return_value=_plan(keywords=["couch"], date_from="2026-06-01", date_to="2026-08-31")),
        patch(QUERY_VECTOR, return_value=None),
    ):
...
def test_search_validation_and_mailbox_scope(auth_client, bob_client, bob_account):
    ...
    assert (
        auth_client.get("/api/search", params={"q": "couch", "account_id": bob_account.id}).status_code == 404
    )
...
def test_search_never_returns_other_users_mail(auth_client, bob_client, db, account, bob_account):
    ...
    with patch(PLAN, return_value=_plan(keywords=["couch"])), patch(QUERY_VECTOR, return_value=None):
        ...
        def ids(client):
            items = client.get("/api/search", params={"q": "couch"}).json()["items"]
            return [i["id"].split(":")[1] for i in items]
...
def test_search_is_rate_limited_per_user(auth_client, bob_client, db, account):
    ...
    with (
        patch.object(mail_api, "ai_limiter", limiter),
        patch(PLAN, return_value=_plan(keywords=["x"])),
        patch(QUERY_VECTOR, return_value=None),
    ):
        ...
        assert (
            bob_client.get("/api/search", params={"q": "couch"}).status_code == 200
...
def test_summarising_a_thread_is_cached_until_it_grows(auth_client, db, account):
    ...
    with patch(THREAD_AI, return_value="<p>SUMMARY</p>") as ai:
        ...
        assert (
            r.status_code == 200
            and r.json()["summary"] == "<p>SUMMARY</p>"
            and r.json()["summary_current"] is True
        )
